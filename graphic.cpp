...
void Engine::drawText(int i, int n, Result latest) {
  ...
  params << "A: " << sim.A << " B: " << sim.B << " C: " << sim.C
         << " D: " << sim.D;
...
Engine::Engine(double A, double B, double C, double D, double x0, double y0,
               int n, bool autoCorrect) {
...
Point Engine::toVideoCoords(double x, double y) {
  // cordinate assolute in cordinate video -> (10,10) è l'angolo in alto a
...
int Engine::windowLoop(int n, bool visualizeText) {
  ...
  filename << "logs/" << std::put_time(std::localtime(&t), "%Y-%m-%d_%H-%M-%S")
           << ".txt";
  ...
  outfile << "A: " << sim.A << " | B: " << sim.B << " | C: " << sim.C
          << " | D: " << sim.D << " (" << n << " steps)\n";
  ...
  while (window->isOpen()) {
    ...
    if (i < n) {
      outfile << "n = " << i << " -> x: " << latest.x << ", y: " << latest.y
              << ", h: " << latest.h << '\n';
